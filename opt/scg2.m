function [x, options, flog, pointlog, scalelog] = scg2(fn, model, f, x, options, gradf, varargin)
...
while (j <= niters)
  ...
  if display > 0
    fprintf(1, 'Cycle %4d  Error %11.10f  Scale %e\n', j, fnow, beta);
  end
...
filename = ['oup\observe'    num2str(fn)  '.mat' ];
